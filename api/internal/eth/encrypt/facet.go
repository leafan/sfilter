...
func GetFacets(c *gin.Context) {
	db := utils.GetChainDatabase(c.Param("chain"))

	options, err := parseFacetOptions(c)
	if err != nil {
		return
	}

	filter := parseFacetFilterOptions(c)

	info, count, err := facet.GetFacets(options, filter, db)
	if err != nil {
		utils.ResFailure(c, 500, err.Error())
		return
	}
	data := struct {
		List  []schema.FacetModel `json:"list"`
...
func parseFacetOptions(c *gin.Context) (*options.FindOptions, error) {
...
func parseFacetFilterOptions(c *gin.Context) *primitive.M {
	filter := bson.M{}
	...
	function := c.DefaultQuery("function", "")
	if function != "" {
