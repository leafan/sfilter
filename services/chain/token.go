...
func GetTokenInfo(address string) (*schema.Token, error) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			...
			name, err2 = getSingleProp(address, "name", getClient(), nil)
			symbol, err3 = getSingleProp(address, "symbol", getClient(), nil)
...
func TEST_TOKEN() {
	...
	name, _ := getSingleProp("0x42D52847BE255eacEE8c3f96b3B223c0B3cC0438", "name", getClient(), nil)
	symbol, _ := getSingleProp("0x42D52847BE255eacEE8c3f96b3B223c0B3cC0438", "symbol", getClient(), nil)

	supply, _ := getSingleProp("0x42D52847BE255eacEE8c3f96b3B223c0B3cC0438", "totalSupply", getClient(), nil)

	log.Printf("\n\n[ TEST ] name: %v, symbol: %v, totalSupply: %v\n\n\n", name, symbol, supply)
}
